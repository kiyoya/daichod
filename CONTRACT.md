...
The release version the daemon reports (`GetBookInfo.shim_version`, `daichod --version`) comes from the `VERSION` file at the repo root, baked in at build time — the image build context excludes `.git`, so the tag itself is not derivable there. Cutting a release therefore has one extra step: bump `VERSION` to the tag being cut in the release commit, then tag. The publish workflow refuses a tag that does not match `VERSION`, so the two cannot drift silently.
